...
export function consolidateSystemInfo(
  ...
  const processSystem = (
    ...
    return {
      ...
      status: {
        code: Math.max(sessionResponse.status.code, systemsResponse.status.code, healthResponse.status.code),
  ...
  return {
    ...
    understudies: sessionResponse.result.understudies.map((u) => processSystem(u, "understudy")),
